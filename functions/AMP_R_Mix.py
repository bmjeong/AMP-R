...
def function_AMP_R_mix(p, flg_buf=True):
    ...
    for idx_iter in range(p.N_iter):
        ...
        for id_a in aa:
            if idx_iter > 50:
                depots[id_a].msg_cal(boards, lkh_=True)
            else:
                depots[id_a].msg_cal(boards, lkh_=False)
        ...
        if idx_iter == 0:
            cvg_msg_rate[idx_iter] = 0
        else:
            tp = 0
            total_task = 0
            for d in range(p.depot_n):
                for route_part in depots[d].route_S:
                    for t in route_part:
                        if abs(log_msg[idx_iter - 1, d, t] - log_msg[idx_iter, d, t]) <= 1e-6:
                            tp += 1
                total_task += depots[d].num_customer
    ...
    result = Results(routes, caps, route_len, vehs, "AMP_R_Mix")
